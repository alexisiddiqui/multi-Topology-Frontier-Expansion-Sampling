...
# @contextmanager
# def cuda_mps_context():
#     """
#     Context manager for handling CUDA MPS startup and shutdown.
#     """
#     try:
#         # Start MPS control daemon
#         subprocess.run(["nvidia-smi", "-i", "0", "-c", "EXCLUSIVE_PROCESS"], check=True)
#         subprocess.run(["nvidia-cuda-mps-control", "-d"], check=True)
#         print("CUDA MPS server started successfully")
#         yield
#     finally:
#         # Shutdown MPS
#         try:
...
#             print(f"Error shutting down MPS: {e}")
#         finally:
#             subprocess.run(["nvidia-smi", "-i", "0", "-c", "DEFAULT"], check=True)
#             print("CUDA MPS server shutdown complete")
...
def process_cycles(systems: List[SystemConfig], num_cycles: int, source_dir: str) -> None:
    ...
    try:
        ...
        max_concurrent = min(4, gpu_memory // 1000)  # Increased memory estimate per simulation
    except:
...
def main():
    ...
    process_cycles(systems, num_cycles=2, source_dir=base_output_dir)
